...
parse_file_test_1() ->
    Text = text(),
    {ok, AST, Refs, Lines} = sourcer_db:parse_file("foo1", Text),
...
get_element_test_000() ->
    ...
    [
        ?_assertMatch([],
                        sourcer_model:get_element_at_pos(Open, <<"foo">>, #{line=>0, character=>0})
                    ),
        ?_assertMatch({ref,{module_def,"foo"},1,13,module,-3,[],false},
                        sourcer_model:get_element_at_pos(Open, <<"foo">>, #{line=>1, character=>1})
                    ),
        ?_assertMatch({ref,{function_def,bar,0},15,3,bar,0,[],false},
                        sourcer_model:get_element_at_pos(Open, <<"foo">>, #{line=>2, character=>1})
                    ),
        ?_assertMatch({ref,{function_def,quz,1},36,3,quz,1,[],false},
                        sourcer_model:get_element_at_pos(Open, <<"foo">>, #{line=>5, character=>1})
                    )
    ].
